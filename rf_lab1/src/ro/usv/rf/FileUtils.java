...
public class FileUtils {
	...
	protected static double[][] readFromFile(String fileName) {

		// Start with an ArrayList<ArrayList<Double>>
		List<ArrayList<Double>> learningSet = new ArrayList<ArrayList<Double>>(); // read file into stream,
		// try-with-resources 
		...
		{ 
			e.printStackTrace(); 
		} // convertArrayList<ArrayList<Double>> to double[][] for performance 
	...
	private static double[][] convertToBiDimensionalArray(List<ArrayList<Double>> learningSet) {
		...
		for (int n = 0; n < learningSet.size(); n++) {
			ArrayList<Double> rowListEntry = learningSet.get(n); // get each row double values
	...
	private static ArrayList<Double> convertLineToLearningSetRow(String line) {
		...
		String[] stringValues = line.split(inputFileValuesSeparator); // we need to convert from string to double
